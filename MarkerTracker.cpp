...
void MarkerTracker::init()
{
    ...
    cv::createTrackbar( "Threshold", kWinName2, &slider_value, max, trackbarHandler, &slider_value);
    
    int bw_sileder_value = bw_thresh;
    cv::createTrackbar( "BW Threshold", kWinName2, &slider_value, max, bw_trackbarHandler, &bw_sileder_value);
...
void MarkerTracker::findMarker( cv::Mat &img_bgr, float resultMatrix[4][16], const vector<int> &__code, vector<bool> &found, bool isStart)
{
